...
func (suite *QueueSuite) TestConnections(c *C) {
	flushConn := OpenConnection("conns-flush", "tcp", "localhost:6379", 1, false)
	flushConn.flushDb()
	flushConn.StopHeartbeat()

	connection := OpenConnection("conns-conn", "tcp", "localhost:6379", 1, false)
	...
	conn1 := OpenConnection("conns-conn1", "tcp", "localhost:6379", 1, false)
	...
	conn2 := OpenConnection("conns-conn2", "tcp", "localhost:6379", 1, false)
...
func (suite *QueueSuite) TestConnectionQueues(c *C) {
	connection := OpenConnection("conn-q-conn", "tcp", "localhost:6379", 1, false)
	c.Assert(connection, NotNil)
...
func (suite *QueueSuite) TestQueue(c *C) {
	connection := OpenConnection("queue-conn", "tcp", "localhost:6379", 1, false)
	c.Assert(connection, NotNil)
	...
	c.Check(queue.Publish("queue-d1", 0), Equals, true)
	c.Check(queue.ReadyCount(), Equals, 1)
	c.Check(queue.Publish("queue-d2", 0), Equals, true)
...
func (suite *QueueSuite) TestConsumer(c *C) {
	connection := OpenConnection("cons-conn", "tcp", "localhost:6379", 1, false)
	c.Assert(connection, NotNil)
	...
	c.Check(queue1.Publish("cons-d1", 0), Equals, true)
	time.Sleep(50 * time.Millisecond)
	...
	c.Check(queue1.Publish("cons-d2", 0), Equals, true)
	time.Sleep(50 * time.Millisecond)
	...
	c.Check(queue1.Publish("cons-d3", 0), Equals, true)
	time.Sleep(50 * time.Millisecond)
	c.Check(queue1.ReadyCount(), Equals, 0)
	c.Check(queue1.UnackedCount(), Equals, 1)
	...
	c.Check(queue1.Publish("cons-d4", 0), Equals, true)
	time.Sleep(50 * time.Millisecond)
	c.Check(queue1.ReadyCount(), Equals, 0)
	c.Check(queue1.UnackedCount(), Equals, 1)
	c.Check(queue1.RejectedCount(), Equals, 1)
	...
	c.Check(queue2.Publish(payload, 0), Equals, true)
	time.Sleep(50 * time.Millisecond)
...
func (suite *QueueSuite) TestMulti(c *C) {
	...
	for i := 0; i < 20; i++ {
		c.Check(queue.Publish(fmt.Sprintf("multi-d%d", i), 0), Equals, true)
	}
...
func (suite *QueueSuite) TestBatch(c *C) {
	connection := OpenConnection("batch-conn", "tcp", "localhost:6379", 1, true)
	queue := connection.OpenQueue("batch-q").(*redisQueue)
	queue.PurgeRejected()
	queue.PurgeReady()

	for i := 0; i < 5; i++ {
		c.Check(queue.Publish(fmt.Sprintf("batch-d%d", i), 0), Equals, true)
	}
...
func (suite *QueueSuite) TestReturnRejected(c *C) {
	connection := OpenConnection("return-conn", "tcp", "localhost:6379", 1, true)
	queue := connection.OpenQueue("return-q").(*redisQueue)
	queue.PurgeReady()

	for i := 0; i < 6; i++ {
		c.Check(queue.Publish(fmt.Sprintf("return-d%d", i), 0), Equals, true)
	}
...
func (suite *QueueSuite) TestPushQueue(c *C) {
	connection := OpenConnection("push", "tcp", "localhost:6379", 1, false)
...
func (suite *QueueSuite) TestConsuming(c *C) {
	connection := OpenConnection("consume", "tcp", "localhost:6379", 1, false)
...
func (suite *QueueSuite) TestStopConsuming_Consumer(c *C) {
	...
	for i := 0; i < deliveryCount; i++ {
		queue.Publish("d"+strconv.Itoa(i), 0)
	}
...
func (suite *QueueSuite) TestStopConsuming_BatchConsumer(c *C) {
	connection := OpenConnection("batchConsume", "tcp", "localhost:6379", 1, false)
...
func (suite *QueueSuite) BenchmarkQueue(c *C) {
	// open queue
	connection := OpenConnection("bench-conn", "tcp", "localhost:6379", 1, false)
	...
	// publish deliveries
	for i := 0; i < c.N; i++ {
		c.Check(queue.Publish("bench-d", 0), Equals, true)
	}
